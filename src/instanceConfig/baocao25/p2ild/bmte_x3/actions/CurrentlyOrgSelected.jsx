...
export const getDataCommon = async (props) => {
    ...
    function setValue() {
        ...
        (async () => {
            let apiDECommulative = "/api/analytics.json?dimension=dx:" +
                stringDECommulative.join(';') +
                "&dimension=ou:" +
                orgUnitSelectedID +
            ...
            let a = await Promise.all(
                [
                    ...chunk(resultDe, 300).reduce((total, bundleDE, idx, arr) => {
            ...
            [...resultDe, ...stringDECommulative].forEach(function (de) {
                let value = 0;
                json.some(searchValue => {
                    let getValue = ({ de, colSpan, disable }) => {
                        ...
                        if (de) {
                            let value = getValueDE({ jsonDhis: searchValue, de })
                            return value
                        } else {
                            return 0
                        }
                    ...
                    if (searchValue.metaData.dimensions.dx.includes(de)) {
                        if (stringDECommulative.includes(de)) {
                            value = getLastesPeriodValue(searchValue, de, orgUnitSelectedID);
                        } else {
                            value = getValue({ de });
                        }
